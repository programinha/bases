...
IA e Large Language Models (modelos de linguagem de larga escala) são máquinas estatísticas que irão produzir o texto mais provável com base no conteúdo usado para treinar essas IAs e os ajustes dados. Se você usar o CoPilot e começar a escrever uma função 'function potencia', ele vai buscar e sugerir a solução mais provável (simples, até simplória). Usando o ChatGPT ele gera conforme a imagem a seguir, uma versão direta e outra mais manual (sem o operador `**`). Em ambos, não são tratados os casos excepcionais (e se base não for numérico? E se expoente não for informado?). Também não nenhum aprendizado aqui, pois, incrivelmente, o aprendizado acontece pelos dedos antes do cérebro. Isto é, você aprende o que você digita -- a famosa memória mecânica, ou em Inglês _muscle memory_. Todo o código gerado por IA precisará de manutenção. Implementar é apenas 1/3 ou menos do esforço, sendo a maior parte dedicada à manutenção. Você não vai lembrar de algo que não escreveu. De algum modo, gerar com IA é um copiar/colar com identificação automática do contexto, não passa pelo seu cérebro, o que a área da psicologia chama de _cognitive offloading_, em outra palavras, livrar-se de usar a cognição, o pensamento. Diversos estudos apontam para a atrofia, a perda de habilidades (ou nem o ganho delas se você ainda está aprendendo), a dificuldade de manter o código no médio e longo prazo. Enfim, usar IA é apenas a dose de dopamina de fazer algo rápido, ter a impressão de produtividade, não obter nenhuma habilidade relevante e terminar não tendo condições de escrever código por si mesmo, nas situações que isso for exigido. Não há problema em usar IA, quando você sabe o que ela está produzindo, é capaz de auditar, corrigir, fazer ajustes que melhoram a solução, pois você sabe programar e seria capaz de resolver o mesmo problema sem IA, pois a está usando por conveniência e não por dependência. 
